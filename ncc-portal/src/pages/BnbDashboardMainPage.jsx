...
function bnbMeUpdateUrl() {
  return API_ORIGIN ? `${API_ORIGIN}/api/bnb/me` : '/api/bnb/me'
}

// SINGLE SOURCE OF TRUTH: /api/bnb/partner/me
function partnerMeUrl() {
  return API_ORIGIN ? `${API_ORIGIN}/api/bnb/partner/me` : '/api/bnb/partner/me'
}

function historyUrl() {
  return API_ORIGIN
    ? `${API_ORIGIN}/api/payments/by-referral?bnb_id=me`
    : '/api/payments/by-referral?bnb_id=me'
}

const BRANDING_ASSET_HOST = (API_ORIGIN || 'http://localhost:8000').replace(/\/$/, '')
...
export default function BnbDashboardMainPage() {
  ...
  const uploadLogoFile = useCallback(async (file) => {
    ...
    try {
      ...
      setBrandLogoUrl(`${BRANDING_ASSET_HOST}${path}`)
      setBrandSavedPreview(false)
      return true
    } catch {
  ...
  const uploadCoverFile = useCallback(async (file) => {
    ...
    try {
      ...
      const path = raw.startsWith('/') ? raw : `/${raw}`
      setBrandCoverUrl(`${BRANDING_ASSET_HOST}${path}`)
      return true
    } catch {
    ...
    try {
      const res = await authFetch(bnbMeUpdateUrl(), {
        ...
        body: JSON.stringify({
          ...
          cover_image_url: brandCoverUrl.trim() || null,
        }),
      })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
  ...
  }, [brandCoverUrl, brandDisplayName, brandLogoUrl])
  ...
  const headerLogoSrc = useMemo(
    () => resolveMediaUrl(brandLogoUrl || me?.logo_url || ''),
    [brandLogoUrl, me?.logo_url],
  )

  const headerCoverSrc = useMemo(() => {
    const u = resolveMediaUrl(brandCoverUrl || me?.cover_image_url || '')
    return u || '/cover-placeholder.jpg'
  }, [brandCoverUrl, me?.cover_image_url])
          ...
          <section style={bigSectionStyle} aria-labelledby="branding-heading">
            ...
            {brandSavedPreview ? (
              ...
              >
                ...
                >
                  {resolveMediaUrl(me?.cover_image_url) ? (
                    <img
                      src={resolveMediaUrl(me?.cover_image_url)}
                      alt=""
                  ...
                  <div style={{ padding: '16px 14px' }}>
                    {resolveMediaUrl(brandLogoUrl || me?.logo_url) ? (
                      <img
                        src={resolveMediaUrl(brandLogoUrl || me?.logo_url)}
                        alt={me?.display_name || 'Logo'}
                        style={{ maxHeight: 48, width: 'auto', objectFit: 'contain', marginBottom: 10 }}
                      />
                    ) : null}
